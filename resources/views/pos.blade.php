...
<div class="row">
	...
	<ul class="price-features">
		<table class="table table-striped">
			...
			<tr>
				...
				<td>
					<form action="{{ url('/cart-update/'.$prod->id) }}" method="post">
						@csrf
						<input type="number" name="quantity" value="{{ $prod->quantity }}" style="width: 40px; ">
						<button type="submit" class="btn btn-sm btn-success" style="margin-top: -2px;"><i class="fas fa-check"></i></button>
					</form>
				</td>
				<td>{{ $prod->price }}</td>
				<td>{{ $prod->price*$prod->quantity }}</td>
				<td><a href="{{ URL::to('/cart-remove/'.$prod->id) }}" ><i class="fas fa-trash-alt text-danger"></i></a></td>
	...
	<div class="pricing-header bg-primary">
		...
		<p>Vate: : {{ Cart::getConditions('10%') }}</p>
		<p><h2 class="text-white">Total:</h2><h1 class="text-white">{{ Cart::getTotal() }}</h1></p>
	    ...
	    <table id="datatable" class="table table-striped table-bordered">
			...
			@foreach($product as $row)
				...
				<td><img src="{{ URL::to($row->product_image) }}" width="60px" height="60px"></td>
					...
					<td>
						<button type="submit" class="btn btn-sm btn-success"><i style="font-size: 30px" class="fas fa-plus"></i></button>
					</td>
				</form>	
				</tr>
			@endforeach   
